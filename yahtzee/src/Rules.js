...
/** Check for small straights. 1234 2345 3456*/

class SmallStraight extends Rule {
  evalRoll = dice => {
    const diceSet = new Set(dice);
    const diceStr = Array.from(diceSet).sort().join("");
    return (diceStr.length >= 4 && /1234|2345|3456/.test(diceStr)) ? this.score : 0;
  }
}
...
const threeOfKind = new SumDistro({ count: 3, description: "3+ of one value, score sum of all dice" });
const fourOfKind = new SumDistro({ count: 4, description: "4+ of one value, score sum of all dice"});

// full house scores as flat 25
const fullHouse = new FullHouse({ score: 25, description: "3 and 2 same values, score 25" });

// small/large straights score as 30/40
const smallStraight = new SmallStraight({ score: 30, description: "4+ values in a row, score 30" });
const largeStraight = new LargeStraight({ score: 40, description: "5 values in a row, score 40" });

// yahtzee scores as 50
const yahtzee = new Yahtzee({ score: 50, description: "All values match, score 50" });
